...
- [x] **release 下载安装**：7z 下载（进度回调）+ 关键文件校验 + 原子安装（`Hooks/<AssemblyVersion>`）+ 写 `version.json`；注：release 包不含 `hashes.json`（远端 `Hash` 用于校验安装后生成的 runtime hashes），已按实测修正
- [ ] **Windows x64 .NET runtime 下载/组装**（`RuntimeVersion` 管理）
- [ ] **Dalamud assets 获取**
- [x] **launch backend 切换**：`launch_game` 支持 `dalamud` 配置时走 Injector（winepath 转换 + Injector 启动 + JSON 解析）；`Launcher::launch` 自动读 `[dalamud].enabled` 并安全降级（版本不匹配时不加载）
