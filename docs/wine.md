...
| `managed` | XIVLauncher 托管路径 → 本项目已下载的（`~/.eorzea/tools/wine/bin/wine64`）→ 下载 |
...
- 默认 prefix 为 `~/.eorzea/prefix`，可用配置项 `prefix` 覆盖。
- 每次启动前 `ensure_prefix()`：
...
- 首次启动时从 CN 镜像下载 `dxvk-async 1.10.1`，解压缓存于 `~/.eorzea/tools/dxvk/`，并把 x64 DLL 复制到 prefix 的 `system32`、x32 复制到 `syswow64`。
...
- 每次启动的 wine/游戏输出重定向到 `~/.eorzea/logs/game-{unix_ts}.log`，不污染终端；CLI 启动成功后会打印该路径。
